...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<tr class="ct_list_pop">
		...
		<td></td>
		<%if ((userVO!=null && userVO.getRole().equals("admin")) || vo.getProTranCode().equals("0")){ %>		
		...
		%>
			판매중
		<%
			}else if (Integer.parseInt(vo.getProTranCode())>0 
										&& (userVO==null || userVO.getRole().equals("user"))){
		%>
			재고 없음
		<%
	...
	<c:forEach var="prod" items="${list}">
		...
		<tr class="ct_list_pop">
			...
			<td></td>
			<c:if test="${(!empty user && user.role=='admin') || prod.proTranCode =='0'}">
				<td align="left">
					<a href="/getProduct.do?prodNo=${prod.prodNo}&menu=${menu}&tranCode=${prod.proTranCode}">${prod.prodName}</a>
				</td>
			</c:if>
			<c:if test="${prod.proTranCode>0 && (empty user || user.role=='user')}">
				<td align="left">
					${prod.prodName}
				</td>
			</c:if>
			...
			<c:if test="${prod.proTranCode=='1' && (!empty user && user.role=='admin')}">
				구매 완료
				<c:if test="${menu == 'manage' }">
					<a href="/updateTranCodeByProd.do?prodNo=${prod.prodNo}&tranCode=2">배송하기</a>
				</c:if>
			</c:if>
			<c:if test="${prod.proTranCode=='2'&& (!empty user && user.role=='admin')}">
				배송중
			</c:if>
			<c:if test="${prod.proTranCode=='3'&& (!empty user && user.role=='admin')}">
				배송 완료
			</c:if>
			</td>	
		</tr>
